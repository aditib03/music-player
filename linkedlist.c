    ...
    while (current && current->next) {
        ...
        while (temp->next != NULL) {
                ...
                strcmp(current->songName, temp->next->songName) == 0)
			{
                ...
                if(duplicate_node->next) duplicate_node->next->prev = temp; 
                free(duplicate_node);
...
//q6 display playlist for given attributes and take option from user to print details of previous song details or next song details
...
void DisplayplaylistDetails(Song* playlist,int index)
{
	...
	if(strcmp(direction, "down") == 0 || strcmp(direction, "Dowm") == 0 || strcmp(direction, "DOWN") == 0)
	{
		Song* tempDown = temp1;
		int tempvalue1=k;
		while(tempvalue1 && tempDown) 
		{
		    tempDown = tempDown->next;
            tempvalue1--;
		}
		if(tempDown == NULL){
			printf("%d th node from down cannot exist. Try giving another value", k); 
		}
		else
		{
    ...
    do
    {
        ...
        switch(choice)
        {
            ...
            case 7:
                ...
                printf("Enter index only in this range (both inclusive)\n");
                scanf("%d", &idx);
